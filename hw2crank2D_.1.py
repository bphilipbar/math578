...
def residual(input):
  ...
  # interior
  output[2:Nx,1:Ny] = input[2:Nx,2:Ny] - rx/2*(input[1:Nx-1,2:Ny]+input[3:Nx+1,2:Ny]-2*input[2:Nx,2:Ny]) - ry*(input[2:Nx,1:Ny-1]+input[2:Nx,3:Ny+1]-2*input[2:Nx,2:Ny]) - RHS
  # almost left
  output
  # top
...
# Time Stepping
for n in range(1,Nt):
   RHS = u[n-1,1:Nx,2:Ny] + rx/2*(u[n-1,0:Nx-1,2:Ny]+u[n-1,2:Nx+1,2:Ny]-2*u[n-1,1:Nx,2:Ny]) + ry*(u[n-1,1:Nx,0:Ny-1]+u[n-1,1:Nx,2:Ny+1]-2*u[n-1,1:Nx,2:Ny])
   RHSleft = u[n-1,1,2:Ny] + rx/2*(u[n-1,2,2:Ny]+u[n-1,2,1:Ny]-2*u[n-1,1,2:Ny]) + ry*(u[n-1,1,1:Ny-1]+u[n-1,1,3:Ny+1]-2*u[n-1,1,2:Ny])
   RHSbottom = u[n-1,1:Nx,1:Ny] + rx/2*(u[n-1,0:Nx-1,1:Ny]+u[n-1,2:Nx+1,1:Ny]-2*u[n-1,1:Nx,1:Ny]) + ry*(u[n-1,1:Nx,0:Ny-1]+u[n-1,1:Nx,2:Ny+1]-2*u[n-1,1:Nx,1:Ny])
